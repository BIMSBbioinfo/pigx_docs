...
* Alignments:
  * Per sample aligned reads: *SAM* and *BAM* files of reads aligned against the
    reference genome, at various stages of trimming and sorting.
  * Per sample unaligned reads: *FASTQ* and *BAM* files not aligning to the
    reference genome.
* SNV files:
  * Per sample *VCF* and *CSV* listing all detected single nucleotide variants
    (SNVs) from the aligned reads.
* VEP reports:
  * Per sample *TXT* / *HTML* report files constituting the
    ...
    * *TXT* file with the raw VEP output.
    * *HTML* report with detailed run statistics.
    * *TXT* file with warnings generated by vep during the run.
    * *CSV* file with most of the raw output parsed into a comma separated table
      for downstream processing.
* Deconvoluted variant abundances:
  * Per sample *CSV* files giving abundance of each of the VOCs as
    * a regular table,
    * in a single row together with metadata (used later to construct summary).
  * Overall summary table *CSV* file of per sample variant abundances with
    sample metadata.
* Mutation abundances:
  * Per sample *CSV* files giving abundance of each signature mutation found and
    meeting the read depth threshold criterium as a single row together with
    metadata (used later to construct summary).
  * Mutation data of non-signature mutations meeting the read depth threshold as
    a *CSV* file.
  * Overall summary table *CSV* file of per sample mutation abundances with
    sample metadata.
* Sample quality:
  * Per sample *TSV* files giving the read depth at each signature mutation
    locus, derived from the *untrimmed* alignment.
  * Per sample *TSV* files giving coverage statistics of the *untrimmed*
    alignment.
  * Per sample *CSV* files aggregating the statistics of genome and mutation
    coverage files in a sample quality table (single row for downstream use).
  * Overall *CSV* file concatenating the per sample quality rows into one table.
* Trimmed *FASTQ*(.gz) files of adapter trimmed reads.
